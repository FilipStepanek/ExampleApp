        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                Rectangle()
                    ...
                    .cornerRadius(2)
                    .animation(.easeInOut, value: value) // Animate changes to the filled rectangle width
                    .gesture(DragGesture(minimumDistance: 0)
                        .onChanged { value in
                            let bounds = geometry.frame(in: .local)
                            let newValue = self.clamp(Double(value.location.x / bounds.width), in: self.range)
                            withAnimation(.easeInOut) {
                                self.value = newValue
                            }
                            self.seekCallback(self.interpolatedValue(from: newValue, in: self.range))
                        }
                    )
            }
        }
